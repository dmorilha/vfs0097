...
struct Key {
  ...
  std::array<uint8_t, 0x20> x_array() const {
    ...
    std::array<uint8_t, 0x20> result = { 0x0, };
    {
      ...
      if (0x20 != result_code) {
        std::cerr << "result code " << result_code << std::endl;
...
static const size_t ASYMMETRIC_KEY_FACTOR_MAX_SIZE = 0x20;
...
std::vector<uint8_t> prf(const std::span<const uint8_t> & key, const std::string & prefix, const std::span<const uint8_t> & original_seed, const std::size_t length) {
...
private:
  ...
  void parse_private_key(TLS & result, const Block & block) {
    ...
    ++iterator;
    assert(DIGEST_SIZE < END - iterator);

    const auto content = block.body.subspan(1, block.body.size() - (DIGEST_SIZE + 1));
    const std::span<const uint8_t, DIGEST_SIZE> digest(END - DIGEST_SIZE, END);
    assert(sha256_hmac_compare(psk_validation_key_, content, digest));

    const size_t AES256_BLOCK_SIZE = 0x10;
    ...
    const std::span<const uint8_t, 0x20> d(output.cbegin(), output.cbegin() + 0x20);
    const std::span<const uint8_t, 0x20> y(output.cbegin() + 0x20, output.cbegin() + 0x40);
    const std::span<const uint8_t, 0x20> x(output.cbegin() + 0x40, output.cbegin() + 0x60);
...
struct ValiditySensors {
  ...
  int bulk_transfer(const uint8_t endpoint, Message & message, const unsigned int timeout = 0) const {
    assert(nullptr != context_);
    assert(nullptr != device_);
    assert(0 < endpoint);
    int transferred = 0;
    if (0 == message.size()) {
      message.resize(/* talvez um pouco maior */ 4096);
    }
    const int result = libusb_bulk_transfer(device_, endpoint, message.data(), message.size(), &transferred, timeout);
    if (0 > result) {
  ...
  int interrupt_transfer(const uint8_t endpoint, Message & message, const unsigned int timeout = 0) const {
    assert(nullptr != context_);
    assert(nullptr != device_);
    assert(0 < endpoint);
    int transferred = 0;
    ...
    const int result = libusb_interrupt_transfer(device_, endpoint, message.data(), message.size(), &transferred, timeout);
    if (0 > result) {
      std::cerr << "libusb_interrupt_transfer error " << static_cast<libusb_error>(result) << std::endl;
    }
    message.resize(transferred);
    return 0 == result ? transferred : result;
  }
  ...
  void setup_tls() {
    std::cout << "Retriving TLS Certificate..." << std::endl;
    {
      ...
      const int result = bulk_transfer(1, vector);
      assert(0 < result);
    }

    {
      ValiditySensors::Message response(4104);
      const int result = bulk_transfer(0x81, response, 3000);
      assert(0 < result);
  ...
  void send(Message message) {
    ...
    const int result = bulk_transfer(1, message_encrypted);
    assert(message_encrypted.size() == result);
  }

  Message receive(std::size_t buffer_size = 0) {
    assert(secure_rx_);
    if (0 == buffer_size) {
      buffer_size = /* talvez um pouco maior */ 4096;
    } else {
      buffer_size = (buffer_size / DIGEST_SIZE + 2) * DIGEST_SIZE + 5;
    }
    Message message(buffer_size);
    {
      const int result = bulk_transfer(0x81, message, 3000);
      assert(message.size() == result);
    }
    assert(0x17 == message[0]);
    assert(0x03 == message[1]);
    assert(0x03 == message[2]);
    std::size_t size = message[3] << 8;
    size |= message[4];
    assert(DIGEST_SIZE < size);
    assert(message.size() == size + 5);
    Message message_decrypted = decrypt(std::span(message.begin() + 5, message.end()));
    assert(validate(message[0], message_decrypted));
    return Message(message_decrypted.begin(), message_decrypted.end() - DIGEST_SIZE);
  }
...
private:
  Message send_client_hello() {
    ...
    Message output(128);
    {
      const int result = bulk_transfer(0x81, output, 3000);
      assert(0 < result);
    }
    return output;
  }
  ...
  Message reply_client_hello() {
...

    {
      ...
      const int result = bulk_transfer(1, message);
    }

    Message output(128);
    {
      const int result = bulk_transfer(0x81, output, 5000);
      assert(0 < result);
    }
    return output;
  }
  ...
  void setup_session_keys() {
...

    {
      ...
      sign_key_ = Message(key_block.begin(),
          key_block.begin() + 0x20);
  ...
  void make_certificate_verify(Message & message) {
    ...
    Sha256 hash = handshake_hash_;
    const std::array<uint8_t, DIGEST_SIZE> digest = hash.digest();
  ...
  void make_client_hello(Message & buffer) {
    ...
    /* The next 2 bytes represent the cipher suites */
...
int main() {
  ...
  sensors.init();
  sensors.find_device(0x138A, 0x97);
  sensors.set_configuration();

  {
    {
      ValiditySensors::Message message{ 0x01, };
      const int result = sensors.bulk_transfer(1, message);
      assert(0 < result);
    }

    {
      ValiditySensors::Message message(1024);
      const int result = sensors.bulk_transfer(0x81, message, 3000);
      assert(0 < result);
    }
  }
  ...
  {
    {
      ValiditySensors::Message message{ 0x19, };
      const int result = sensors.bulk_transfer(1, message);
      assert(0 < result);
    }
  ...
  {
    {
      ValiditySensors::Message message{ 0x43, 0x02, };
      const int result = sensors.bulk_transfer(1, message);
      assert(0 < result);
    }
  ...
  {
    {
      ValiditySensors::Message message{
  ...
  { /* GREEN LED ON */
    {
      ValiditySensors::Message message{
        0x39, 0x20, 0xbf, 0x02, 0x00, 0xff, 0xff, 0x00, 0x00, 0x01,
        0x99, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x99, 0x99, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0x00, 0x00, 0x00,
        0x99, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      ...
      sensors.send(message);
    }
    {
      const ValiditySensors::Message message = sensors.receive(2);
      assert(2 == message.size());
      assert(0x00 == message[0]);
      assert(0x00 == message[1]);
    }
  }
  ...
  { /* ACTIVATE SEQUENCE 2 */
    ...
    {
      const ValiditySensors::Message message = sensors.receive(6);
      assert(6 == message.size());
    }
  }
  ...
  for (int i = 1; 0 < i; --i) {
    ...
    {
      const ValiditySensors::Message message = sensors.receive(10);
      assert(10 == message.size());
    }
  }
  ...
  for (int i = 1; 0 < i; --i) {
    ...
    {
      const ValiditySensors::Message message = sensors.receive(108);
      assert(108 == message.size());
    }
  }
  ...
  { /* IDENTIFY PROGRAM */
    {
      ValiditySensors::Message message{
        ,
        ...
        ,
    ...
    {
      const ValiditySensors::Message message = sensors.receive();
      assert(2366 == message.size());
    }
  }
  ...
  for (int i = 0; 5 > i; ++i) {
    ...
    switch(i) {
    case 1:
      std::cout << "Press your finger into the device..." << std::endl;
      timeout = 30000;
      break;

    case 4:
      std::cout << "Remove your finger from the device..." << std::endl;
      timeout = 30000;
      break;
    ...
    ValiditySensors::Message message;
    const int result = sensors.interrupt_transfer(0x83, message, timeout);
    assert(0 < result);
  }
...

  {
    {
      ValiditySensors::Message message{
	0x5e, 0x02, 0xff, 0x03, 0x00, 0x05, 0x00, 0x01, 0x00, 0x00,
        0x00, 0x00, 0x00,
      };
      sensors.send(message);
    }
  ...
  {
    std::array<uint8_t, 5> IDENTIFIED{ 0x03, 0x00, 0x01, 0x00, 0xdb, };
    ValiditySensors::Message message;
    const int result = sensors.interrupt_transfer(0x83, message, 3000);
    assert(0 < result);
    if (std::equal(message.begin(), message.end(), IDENTIFIED.begin(), IDENTIFIED.end())) {
      std::cout << "Your fingerprint was correctly identified and it is contained into the fingerprint reader's database." << std::endl;
      return_value = 0;
    } else {
      std::cerr << "Fingerprint was not found!" << std::endl;
      { /* LED FLASHES */
        {
          ValiditySensors::Message message{
